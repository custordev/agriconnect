...
const FooterColumn: React.FC<{
  ...
  <div>
    <h3 className="font-semibold text-[#423d16] mb-4">{title}</h3>
    <ul>
      {links.map((link, index) => (
        <li key={index} className="mb-2">
          <Link
            href={link.href}
            className="text-gray-600 hover:[#423d16] transition duration-300"
          >
            {link.label}
          </Link>
        </li>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          <div>
            <Link href="/" className="flex items-center gap-2">
              <GiFruitBowl className="h-6 w-6 text-[#423d16]" />
              <span className="text-2xl font-bold text-[#423d16]">
                Agriconnect
              </span>
            </Link>
          </div>
